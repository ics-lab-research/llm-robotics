...
def code_tokenize(code_string):
    ...
    try:
        for token_info in tokenize.generate_tokens(
            io.StringIO(code_string).readline
        ):
            if (
                token_info.type != tokenize.COMMENT
                and token_info.type != tokenize.ENDMARKER
...
def calculate_bleu_score(reference_code, hypothesis_code):
    ...
    """
    reference_tokens = [
        code_tokenize(ref) for ref in reference_code
...
# calculate the score
score1 = calculate_bleu_score(
    reference_code_examples, hypothesis_code_example_1
)
score2 = calculate_bleu_score(
    reference_code_examples, hypothesis_code_example_2
)
print(
    f"BLEU Score 1: {score1:.4f}"
)  # print the score of hypothesis code example 1
print(
    f"BLEU Score 2: {score2:.4f}"
)  # print the score of hypothesis code example 2
